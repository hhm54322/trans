...
- CAD 主索引图默认以 3 张全分辨率图合并为一次视觉请求；每张仅 3 行的高清复核图则以 4 张合并，失败时自动降级为单图。Tesseract 与 Paddle 按容器可见 CPU 分配工作线，Paddle `auto` 只在至少 8 核、12 GiB 的运行环境启用两个常驻预测器，低配服务器自动保持 1。同一文档内只有“字形像素指纹+本地泰文提示”均完全匹配的行才会复用已确认原文。
