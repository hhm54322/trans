...
- 按浏览器匿名身份隔离的翻译历史、复制译文和格式化文件再次下载
- 桌面端与手机 H5 响应式界面
...
APP_LAYOUT_TRANSLATION_CONCURRENCY=5
...
- CAD 主索引图默认以 3 张全分辨率图合并为一次视觉请求；每张仅 3 行的高清复核图则以 4 张合并，缺少 ID 时只补发对应原像素行，整组失败时先递归对半拆分，必要时才降级为单图。6400px 高清复核底图会与本地 OCR 提前并行生成，但仍保持原分辨率与裁切逻辑。Tesseract 与 Paddle 按容器可见 CPU 分配工作线，两者均可用 `APP_CAD_*_WORKERS` 限制并发但不减少识别轮次；Paddle `auto` 只在至少 8 核、12 GiB 的运行环境启用两个常驻预测器，低配服务器自动保持 1。Paddle 与 Tesseract 命中同一行时，只有 Paddle 框的高清复核成功后才会替换原候选。同一文档内只有“字形像素指纹+本地泰文提示”均完全匹配的行才会复用已确认原文；并发页可合并正在进行的相同识别，剩余 PNG 行会以原像素重新紧凑打包。
...
服务器会通过 `HttpOnly` Cookie 为每个浏览器签发持久匿名身份，翻译历史、文件下载和进行中的文档任务都按该身份校验。无归属字段的旧历史在升级后默认不展示，避免将旧共享记录泄露给任意访问者。
...
- 当前历史按浏览器匿名身份隔离；清除 Cookie 或更换浏览器后不会自动找回旧记录。若需跨设备同步和企业租户权限，仍需接入登录和账号级租户。
